...
func remoteShutdown(ip string) error {
	conn, err := net.Dial("tcp", ip+":9090") //ip  TCP ip
	if err != nil {
		log.Printf("Failed to connect to slave %s: %v", ip, err)
		return err
	}
	defer conn.Close()

	_, err = fmt.Fprintln(conn, "shutdown")
	if err != nil {
		log.Printf("Failed to send shutdown command to %s: %v", ip, err)
		return err
	}

	log.Printf("Shutdown command sent to %s", ip)
	return nil
}
